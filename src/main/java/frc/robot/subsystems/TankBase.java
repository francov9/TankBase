...
public class TankBase extends SubsystemBase{
    ...
    private final DCMotorSim lMotorSim = new DCMotorSim(
    LinearSystemId.createDCMotorSystem(
        DCMotor.getKrakenX60(1), 0.001, 1),//😊
    DCMotor.getKrakenX60(1)
    );
    private final DCMotorSim rMotorSim = new DCMotorSim(
    LinearSystemId.createDCMotorSystem(
        DCMotor.getKrakenX60(1), 0.001, 1),
    DCMotor.getKrakenX60(1)
    );
    ...
    private void drive(DoubleSupplier speed, DoubleSupplier turn, boolean run){
        System.out.println("Speed: " + speed.getAsDouble() + " Turn: " + turn.getAsDouble());
        lMotor.set(run ? 1 : 0 * (speed.getAsDouble() - turn.getAsDouble()));
        rMotor.set(run ? 1 : 0 * (-(speed.getAsDouble() + turn.getAsDouble())));
    }

    public Command driveCommand(DoubleSupplier speed, DoubleSupplier turn, boolean run){
        return runOnce(
            () -> drive(speed, turn, run));
    }
   ...
   RtalonFXSim.setRotorVelocity(rMotorSim.getAngularVelocity());
    rMotorSim.update(0.020); // assume 20 ms loop time
        ...
        drivetrainSim.setInputs(
            lMotorSim.getInputVoltage() * lMotor.get(), 
            rMotorSim.getInputVoltage() * rMotor.get()
        );
        drivetrainSim.update(0.02);
        m_gyroSim.setAngle(-drivetrainSim.getHeading().getDegrees()); //heading is negative bc hover over getHeading man
    }
}
